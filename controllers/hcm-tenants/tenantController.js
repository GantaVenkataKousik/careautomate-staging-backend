...
export const getTenantNamesByCompany = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Tenant names and IDs fetched successfully",
      response: tenants,
    });
  } catch (error) {
    console.error("Error in getTenantNamesAndIds:", error);
    return res.status(500).json({
      success: false,
      message: "Internal server error while fetching tenant names and IDs",
    });
  }
};
