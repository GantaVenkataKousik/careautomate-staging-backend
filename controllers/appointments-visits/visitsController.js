...
export const createVisit = async (req, res) => {
    try {
        ...
        if (!creator) {
            return res.status(303).json({ success: false, message: 'Creator not found' });
        }
        ...
        if (creator.role === 2) {
            ...
            return res.status(303).json({ success: false, message: 'Tenants cannot create visits' });
        }
        ...
        if (serviceTracking) {
            serviceTracking.workedUnits = (serviceTracking.workedUnits || 0) + unitsUsed;
            serviceTracking.unitsRemaining -= unitsUsed;
            serviceTracking.scheduledUnits = Math.max(0, serviceTracking.scheduledUnits - unitsUsed);
            // Add hcmId to hcmIds array if not already present
            if (!serviceTracking.hcmIds.some(hcm => hcm.hcmId && hcm.hcmId.toString() === newVisit.hcmId)) {
                serviceTracking.hcmIds.push({ hcmId: newVisit.hcmId, serviceDetails: [{ dateOfService: newVisit.date, workedUnits: unitsUsed }] });
            }
            else {
                serviceTracking.hcmIds.find(hcm => hcm.hcmId && hcm.hcmId.toString() === newVisit.hcmId).serviceDetails.find(service => service.dateOfService === newVisit.date).workedUnits += unitsUsed;
        ...
        res.status(200).json({
            success: true, message: 'Visit created successfully', response: {
                "New Visit": newVisit,
...
export const getVisits = async (req, res) => {
    const { companyId } = req.params;
    try {
        const visits = await Visits.find({ companyId }).populate({
            path: 'creatorId',
            select: 'name email role',
            model: 'causers'
        }).populate({
        ...
        res.status(200).json({
            success: true, message: 'Visits fetched successfully', response: {
                visits: visits
            }
        });
    } catch (error) {
...
export const updateVisit = async (req, res) => {
    ...
    try {
        ...
        if (!visit) {
            return res.status(400).json({ success: false, message: 'Visit not found' });
        }
        const updatedVisitData = {
            ...
            travelWithoutTenant: updateData?.travelWithoutTenant || visit.travelWithoutTenant,
            ...
            reasonForRejection: updateData?.reasonForRejection || visit.reasonForRejection,
            companyId: updateData?.companyId || companyId,
        }
        const updatedVisit = await Visits.findByIdAndUpdate(id, updatedVisitData, { new: true });
        ...
        let serviceRecord;
        if (serviceTracking) {
            const durationInMinutes = (updatedVisit.endTime - updatedVisit.startTime) / 60000;
            let unitsUsed = durationInMinutes / 15; // 15 minutes = 1 unit
            unitsUsed = parseFloat(unitsUsed.toFixed(2)); // Round to two decimal places

            if (unitsUsed > serviceTracking.unitsRemaining) {
                return res.status(400).json({ success: false, message: 'Units remaining is less than the units used' });
            }
            serviceTracking.workedUnits = (serviceTracking.workedUnits || 0) + unitsUsed;
            serviceTracking.unitsRemaining -= unitsUsed;
            serviceTracking.scheduledUnits = Math.max(0, serviceTracking.scheduledUnits - unitsUsed);
            const hcms = serviceTracking.hcmIds.find(hcm => hcm.hcmId && hcm.hcmId.toString() === updatedVisit.hcmId);
            if (hcms) {
                hcms.serviceDetails.find(service => service.dateOfService === updatedVisit.date).workedUnits += unitsUsed;
            }
            serviceRecord = await serviceTracking.save();
        }

        return res.status(200).json({
            success: true, message: 'Visit updated successfully', response: {
...
export const deleteVisit = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            success: true, message: 'Visit deleted successfully', response:
            {
...
export const visitsWaitingForApproval = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            success: true, message: "Visits waiting for approval fetched successfully", response: {
                visits: visits,
                count: visits.length
            }
        });
...
export const markVisitAsApproved = async (req, res) => {
    ...
    try {
        ...
        const user = await users.findById(new mongoose.Types.ObjectId(visit.tenantId));
        ...
        if (!tenantInfo) {
            return res.status(400).json({ success: false, message: 'Tenant information not found' });
        }

        const service = await ServiceTracking.findOne({ tenantId: visit.tenantId, serviceType: visit.serviceType, companyId });
        if (!service) {
            return res.status(400).json({ success: false, message: 'Service tracking record not found' });
        }
        // Create a new bill
