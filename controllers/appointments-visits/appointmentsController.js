...
export const filterAppointments = async (req, res) => {
  try {
    ...
    filteredAppointments.forEach((appointment) => {
      ...
      groupedAppointments[statusGroup][year][month][date].push(
...
export const fetchAppointments = async (req, res) => {
  const { companyId } = req.params;
  try {
    const appointments = await appointments
      .find({ companyId })
      .populate({
        path: "hcmId",
        select: "name email phoneNo",
...
export const updateAppointment = async (req, res) => {
  ...
  try {
    ...
    const updatedAppointment = {
      ...
      serviceType:
        updatedAppointmentData?.serviceType || appointment?.serviceType,
      methodOfContact:
...
export const deleteAppointment = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Appointment and associated visit deleted successfully",
    });
  } catch (error) {
...
export const createAppointment = async (req, res) => {
  try {
    ...
    const { appointment, companyId } = appointmentData;
    ...
    if (
      appointment.methodOfContact === "remote" &&
    ...
    const populatedAppointment = await appointments
    ...
    if (!serviceTracking) {
      ...
      if (hcmEntry) {
        ...
        const serviceDetail = hcmEntry.serviceDetails.find(
          (service) => service.dateOfService === appointment.date
        );

        if (serviceDetail) {
          // Update workedUnits and scheduledUnits
          serviceDetail.workedUnits += unitsToAdd;
          serviceDetail.scheduledUnits = Math.max(
            0,
            serviceDetail.scheduledUnits - unitsToAdd
          );
        } else {
