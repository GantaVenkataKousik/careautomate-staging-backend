...
const getTenantPersonalInfoReports = async (req, res) => {
    ...
    try {
        ...
        const tenants = await users.find({ role: 0, companyId }).select('_id info_id').lean();
        ...
        for (const tenant of tenants) {
            const info = await tenantInfo.findById(tenant.info_id).select('personalInfo.firstName personalInfo.lastName personalInfo.maPMINumber address.city address.state address.zipCode designate').lean();
...
const getServiceTrackingPlanReports = async (req, res) => {
    ...
    try {
        ...
        const formattedReports = serviceTrackingReports.map(report => ({
            ...
            dateRange: `${report.startDate.toISOString().split('T')[0]} to ${report.endDate.toISOString().split('T')[0]}`,
...
const getHcmPersonalInfoReports = async (req, res) => {
    ...
    try {
        ...
        const hcms = await users.find({ role: 1, companyId }).select('_id info_id').lean();
        ...
        for (const hcm of hcms) {
            const info = await hcmInfo.findById(hcm.info_id).select('personalInfo.firstName personalInfo.lastName addressInfo.addressLine1 addressInfo.city addressInfo.state addressInfo.zipCode employmentInfo.hireDate loginInfo.email').lean();
            if (info) {
                hcmDetails.push({
                    userId: hcm._id,
                    firstName: info.personalInfo.firstName,
...
const numberOfTenantsHcms = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Number of tenants and HCMs fetched successfully", response: { tenants, hcms } });
    } catch (error) {
...
export { getTenantPersonalInfoReports, getServiceTrackingPlanReports, getTenantVisitComplianceReports, getHcmPersonalInfoReports, numberOfTenantsHcms };
