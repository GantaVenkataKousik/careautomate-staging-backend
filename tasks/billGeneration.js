...
import BillPending from '../models/bills/billsPending.js';
...
const generateEdiFiles = async () => {
    try {
        // Fetch all pending bills
        const pendingBills = await BillPending.find({});
        ...
        for (const pendingBill of pendingBills) {
            // Retrieve the full bill details using the bill ID
            const bill = await Bill.findById(pendingBill.bill);

            if (!bill) {
                console.log(`Bill not found for ID: ${pendingBill.bill}`);
                continue;
            }
        ...
        console.error('Error generating EDI files:', error);
    } finally {
